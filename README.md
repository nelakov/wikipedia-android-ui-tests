...
# Wikipedia Android UI Tests

**Mobile UI tests for the [Wikipedia](https://github.com/wikimedia/apps-android-wikipedia) Android app.
The same suite runs on BrowserStack cloud, a local emulator, or a real device over USB.**
...
- Devices and credentials live in `.properties` files, bound type-safely via the [Owner](https://matteobaccan.github.io/owner/) library.
- A shared `LocalAndroidDriver` (template method) backs both the emulator and the real device; subclasses differ only in which config they supply.
- Allure reporting with `@Step` annotations, plus an automatic screenshot and page source on every test.
- A parameterized Jenkins job runs the suite on demand.
- Stack: Java 25, Gradle 9.6 (Kotlin DSL), JUnit 6, Selenide 7, Appium 10.
...
- Onboarding screens: header text across all 4 pages
- Search by query
- Add a new language (and verify the languages list grows)
- Verify the active interface language

## Architecture

The `deviceProvider` system property selects a Selenide `WebDriverProvider` by class name; Selenide
instantiates it to build the Appium driver. The local emulator and the physical device share one base class.
...
Config `.properties` are **git-ignored**, so create the one for your target. The APK auto-downloads if missing.
...
> resources **root**; the BrowserStack and Samsung configs live under `config/`.
